...
export async function getWeekPendingGoals() {
  ...
  const goalCompetionCounts = db.$with('goal_completion_counts').as(
    db.select({
      ...
      gte(goalCompletions.createdAt, firstDayOfWeek), lte(goalCompletions.createdAt, lastDayOfWeek)))
      .groupBy(goalCompletions.goalId)
  )

  const pendingGoals = await db.with(goalsCreatedUpToWeek, goalCompetionCounts)
    .select(
      {
    ...
    .leftJoin(goalCompetionCounts, eq(goalCompetionCounts.goalId, goalsCreatedUpToWeek.id))
